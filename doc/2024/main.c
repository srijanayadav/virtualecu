...
// 	CAN_0.BUF[8].MSG_ID.B.EXT_ID = 0x7299; // Extended ID (if needed)

// 	// Set the length of the payload (up to 8 bytes)
// 	CAN_0.BUF[8].CS.B.LENGTH = len;

// 	// Set the frame type to extended (1 for extended frame, 0 for standard)
// 	CAN_0.BUF[8].MSG_CS.B.IDE = 1; // 1 means Extended Frame

// 	// Request the CAN controller to transmit the message
// 	CAN_0.BUF[8].CS.B.CODE = 0xC; // Transmit request
...
void can_send_speed(int value)
{
	...
	// char message[50];
	// // Format the string and store it in the variable 'message'
    // sprintf(message, "Speed : %d Km/h", speed);

    // // Print the stored string
    // printf("%s\n", message);
	// printf("value of speed is %d",speed);
...
void can_send_lights_status(int left_indicator, int right_indicator)
{
    ...
    if (indicator_status == 0x01 && can_message_11_flag) {
		const char *message = "LeftIndicator On";
...
int main(void)
{
	...
	/* TO-DO: your task implementations **************************************/
	// Start PIT timer channel 1 with a 100ms period
    // Configure and start timer channel 1 for 1-second interval
        // PIT_ConfigureTimer(1, 1000); // 1 second = 1000 milliseconds
        // PIT_StartTimer(1);
